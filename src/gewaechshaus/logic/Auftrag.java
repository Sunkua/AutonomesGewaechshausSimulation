...
public class Auftrag extends Observable implements Observer {
	...
	/**
	 * Gibt einen einzelnen Unterauftrag zurück. Entfernt dabei den Unterauftrag
	 * aus der Queue
	 *
	...
	/**
	 * Sucht den nächsten freien Roboter und führt mit diesem den nächsten
	...
	public void naechstenUnterauftragAusfuehren(Roboter r) throws Exception {
		...
		if (aktiveUnterauftraege.size() <= roboterleitsystem.getRoboter().size()) {
			...
			throw new Exception(
					"Mehr Unteraufträge aktiv als Roboter im System. Fehler wahrscheinlich auf Multithreading zurückzuführen");
	...
	/**
	 * Erstellt ein Runnable auf Basis eines Unterauftrags. Das Runnable prüft
	...
	private Runnable unterauftragsRunnableErstellen(Unterauftrag uAuftrag) {
		return () -> {
			if (uAuftrag.getStatus().equals(UnterauftragsStatus.beendet)) {
				// Unterauftrag als Observer entfernen, damit Ausführen nicht
				// mehr bei jedem Schritt getriggert wird
				uhr.deleteObserver(uAuftrag);
				Roboter roboter = uAuftrag.getRoboter();
				roboter.setUnterauftrag(null);
				uAuftrag.deleteObservers();
				// Roboterleitsystem benachrichtigen, damit es nächsten
				// Unterauftrag anstoßen kann
	...
	/**
	 * Update vom Observer Überprüft den Status der Unterauftrags. Ist der
	 * Unterauftrag abgeschlossen wird der Nächste angestoßen. Enthält die Queue
	...
	public void update(Observable o, Object arg) {
		if (o instanceof Unterauftrag) {
			...
			Runnable runnable = () -> {
				if (uAuftrag.getStatus().equals(UnterauftragsStatus.beendet)) {
					// Unterauftrag als Observer entfernen, damit Ausführen
					// nicht mehr bei jedem Schritt getriggert wird
					uhr.deleteObserver(uAuftrag);
					Roboter roboter = uAuftrag.getRoboter();
					roboter.setUnterauftrag(null);
					uAuftrag.deleteObservers();
